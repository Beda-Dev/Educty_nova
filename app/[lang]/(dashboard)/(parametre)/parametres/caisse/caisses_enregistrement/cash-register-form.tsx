...
export default function CashRegisterForm({ onSuccess }: CashRegisterFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const { setCashRegisters } = useSchoolStore();
  const router = useRouter();

  const { register, handleSubmit, formState: { errors }, reset } = useForm<CashRegisterFormValues>({
