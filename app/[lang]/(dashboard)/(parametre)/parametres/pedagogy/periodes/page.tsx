...
const PeriodsPage = () => {
  ...
  const createPeriods = async () => {
    setIsLoading(true);
    try {
      const periodsToCreate = periodType === "semester"
        ? [
      ...
      // Création séquentielle des périodes
      for (const period of periodsToCreate) {
        ...
        if (!response.ok) {
          throw new Error(`Erreur lors de la création de la période: ${period.label}`);
        }
      }

      // Recharger les périodes après création
      await fetchPeriodBegin();
      
      toast({
        title: "Succès",
        description: `Les ${periodType === "semester" ? "semestres" : "trimestres"} ont été créés avec succès.`,
        color: "success",
      });
  ...
  return (
    <>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Ajouter de nouvelles périodes</DialogTitle>
            <DialogDescription>
              Sélectionnez le type de périodes à ajouter.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <Select
              value={periodType}
              onValueChange={(value: "semester" | "trimester") => setPeriodType(value)}
              disabled={isLoading}
            >
              <SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={isLoading}
            >
              {isLoading ? "Ajout..." : "Ajouter les périodes"}
            </Button>
            </div>
          </DialogFooter>
        </DialogContent>
