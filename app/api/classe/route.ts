...
export async function GET(request: NextRequest) {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/classe`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = await response.json();
    if (!response.ok) {
      throw new Error(data.message || "Erreur lors de la création de la classe");
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/classe/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
    });
    const data = await response.json();
    if (!response.ok) {
      throw new Error(data.message || "Erreur lors de la mise à jour de la classe");
    }
    return NextResponse.json(data, { status: 200 });
  } catch (error: any) {
